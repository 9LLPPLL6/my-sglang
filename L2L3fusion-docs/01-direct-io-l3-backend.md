...
| L1/L2 命中（数据已在显存或主机内存） | 51 ms |

看第二行：**原来的 L3 命中（924 ms）和重算（931 ms）一样慢**。
辛辛苦苦把 KV 存到磁盘上，一分钱便宜都没占到。这就是"L3 太慢"的具体形态。

新后端快了 2.08 倍，终于比重算划算了。但离 51 ms 还有距离。

> 最后一行 L1 和 L2 实测都是 51-52 ms，分不开——因为这段延迟的构成是：
> 固定开销 28.7 ms（发一个 11 字符的请求就要这么久）+ 约 23 ms（tokenize 这个
> 40 KB 的长 prompt，加上把 1.21 GiB KV 从主机拷进显存）。
> 主机→显存的 PCIe 带宽实测 52.8 GiB/s，1.21 GiB 只要 22.9 ms，
> 所以 L2 相对 L1 多出来的那点开销淹没在噪声里了。
>
> 这个分解很重要：**444 ms 里有 355 ms 是纯粹的读盘时间，那才是全部的差距**。
> 详见 `02-...` 阶段文档里的差距分析。
